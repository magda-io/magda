...
| magda-postgres.postgresql.tls.certificatesSecret | string | `"session-db-postgresql-crt"` | Must match `fullnameOverride` above: the postgresql subchart does not run `certificatesSecret` through `tpl`, so it cannot be derived automatically. See `magda-postgres/values.yaml` for the full explanation. |
