...
| helmetPerPath | object | `{"/assets/alasql.html":{"contentSecurityPolicy":{"scriptSrc":["'self'","'unsafe-eval'"]}}}` | Allow optionally to set different helmet config per request path. e.g. you can specify different config for path `/routeA/abc` with: helmetPerPath:   "/routeA/abc":      referrerPolicy:       policy: strict-origin-when-cross-origin |
| helmetPerPath."/assets/alasql.html" | object | `{"contentSecurityPolicy":{"scriptSrc":["'self'","'unsafe-eval'"]}}` | allow alasql to compile SQL query at frontend in its separate window/iframe |
