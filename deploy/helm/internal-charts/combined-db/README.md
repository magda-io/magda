...
|------------|------|---------|
| file://../magda-postgres | magda-postgres | 7.0.0-pr.3637.2 |
...
| magda-postgres.postgresql.tls.certificatesSecret | string | `"combined-db-postgresql-crt"` | Must match `fullnameOverride` above: the postgresql subchart does not run `certificatesSecret` through `tpl`, so it cannot be derived automatically. See `magda-postgres/values.yaml` for the full explanation. |
