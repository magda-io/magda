...
| global.postgresql.postgresqlUsername | string | `"postgres"` | PostgreSQL username used by DB migrator jobs. For in-cluster PostgreSQL, `postgres` remains the default superuser. For external DB modes (`global.useAwsRdsDb` / `global.useCloudSql`), if this value is left as `postgres` and `global.dbUser` is non-`postgres`, migrators will automatically use `global.dbUser` to avoid mismatched default credentials. If you use a non-`postgres` migrator user, ensure it has permissions to run migration scripts (for example, create DB/schema roles as required by the migrators). |
