...
![Version: 7.0.0-pr.3791.0](https://img.shields.io/badge/Version-7.0.0--pr.3791.0-informational?style=flat-square)
...
## PostgreSQL client TLS and CA verification

`global.postgresql.client.sslmode` controls how every Magda service, DB
migrator Job and the `registry-db` auto-vacuum CronJob connect to PostgreSQL:
`disable`, `require` (the default), `verify-ca` and `verify-full` are
supported (`prefer`/`allow` are rejected at render time — see the
[AWS deployment guide](https://github.com/magda-io/magda/blob/main/docs/docs/deploy-to-aws.md#postgresql-client-ssltls-mode-sslmode)).
**Exception:** the external authentication plugins (`magda-auth-*`) do not yet
read this value — they connect to the session DB in plaintext regardless of
`sslmode`, a pre-existing gap tracked separately, not something `verify-full`
newly guarantees against.

**`verify-ca`/`verify-full` require `global.postgresql.client.sslRootCertSecret.name`.**
Helm fails to render — with an explanatory guard message, not a runtime crash
loop — if you set either `verify-` mode without also pointing
`sslRootCertSecret.name` at a Secret holding your database's CA certificate.
This holds **even when the server's CA is a publicly-trusted root** (for
example Azure Database for PostgreSQL's DigiCert Global Root G2): Magda's DB
migrator image ships a libpq older than version 16, which has no
`sslrootcert=system` support, so there is deliberately no system/bundled
trust-store fallback to fall back to. Supply your provider's CA bundle
regardless — AWS RDS's `global-bundle.pem`, Azure's DigiCert Global Root G2 /
Microsoft RSA Root CA 2017, or (when connecting directly rather than through
`global.useCloudSql`'s plaintext proxy sidecar) Cloud SQL's `server-ca.pem` —
and create the Secret before installing. See the
[AWS deployment guide](https://github.com/magda-io/magda/blob/main/docs/docs/deploy-to-aws.md#verify-ca--verify-full-the-ca-secret-is-mandatory-with-no-fallback)
for a worked RDS example and the
[Helm helper contracts](https://github.com/magda-io/magda/blob/main/docs/docs/helm-helper-contracts.md#the-postgresql-client-ca-delivery-helpers)
doc for how the CA is delivered to each workload.
...
|------------|------|---------|
| file://../internal-charts/admin-api | admin-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/apidocs-server | apidocs-server | 7.0.0-pr.3791.0 |
| file://../internal-charts/authorization-api | authorization-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/authorization-db | authorization-db | 7.0.0-pr.3791.0 |
| file://../internal-charts/cloud-sql-proxy | cloud-sql-proxy | 7.0.0-pr.3791.0 |
| file://../internal-charts/combined-db | combined-db | 7.0.0-pr.3791.0 |
| file://../internal-charts/content-api | content-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/content-db | content-db | 7.0.0-pr.3791.0 |
| file://../internal-charts/correspondence-api | correspondence-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/elasticsearch | elasticsearch | 7.0.0-pr.3791.0 |
| file://../internal-charts/gateway | gateway | 7.0.0-pr.3791.0 |
| file://../internal-charts/indexer | indexer | 7.0.0-pr.3791.0 |
| file://../internal-charts/ingress | ingress | 7.0.0-pr.3791.0 |
| file://../internal-charts/opensearch-dashboards | opensearch-dashboards | 7.0.0-pr.3791.0 |
| file://../internal-charts/opensearch | opensearch | 7.0.0-pr.3791.0 |
| file://../internal-charts/priorities | priorities | 7.0.0-pr.3791.0 |
| file://../internal-charts/rds-dev-proxy | rds-dev-proxy | 7.0.0-pr.3791.0 |
| file://../internal-charts/registry-api | registry-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/registry-db | registry-db | 7.0.0-pr.3791.0 |
| file://../internal-charts/search-api-node | search-api-node | 7.0.0-pr.3791.0 |
| file://../internal-charts/search-api | search-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/semantic-search-api | semantic-search-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/session-db | session-db | 7.0.0-pr.3791.0 |
| file://../internal-charts/storage-api | storage-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/tenant-api | tenant-api | 7.0.0-pr.3791.0 |
| file://../internal-charts/tenant-db | tenant-db | 7.0.0-pr.3791.0 |
| file://../internal-charts/web-server | web-server | 7.0.0-pr.3791.0 |
| file://../magda-common | magda-common | 7.0.0-pr.3791.0 |
| oci://ghcr.io/magda-io/charts | magda-embedding-api | 1.1.0 |
| oci://ghcr.io/magda-io/charts | preview-map(magda-preview-map) | 2.0.0-alpha.0 |
...
| global.postgresql.allowDefaultExternalDbPostgresUser | bool | `false` | When using external DB services (`useCloudSql` or `useAwsRdsDb`), chart rendering fails by default if `auth.username` is left as `postgres` to prevent accidental use of the wrong privileged account. Set this to `true` only when your external privileged account is intentionally named `postgres`. |
| global.postgresql.auth.existingSecret | string | `"db-main-account-secret"` | the secret that contains privileged PostgreSQL account password. The password will be loaded from key "postgresql-password" of the secret data. Previously (before v1.0.0), we used to load the password from "cloudsql-db-credentials" secret `password` field when use cloud provider DB services. Since v1.0.0, our helm chart can auto-create the secret and copy the content of "cloudsql-db-credentials" secret when: <ul>   <li> "autoCreateSecret" is set to true</li>   <li> "cloudsql-db-credentials" exists </li> </ul> for backward compatibility purposes. <br/><br/> Please note: when supplying the secret manually, it's recommend to set `"helm.sh/resource-policy": keep` annotation on the secret to avoid the secret being removed by Helm.<br/><br/> e.g. you can set the annotation of the secret with the following command: `kubectl annotate --namespace [install namespace] secret db-main-account-secret 'helm.sh/resource-policy'=keep` <br/><br/> When you supply this secret yourself AND use a non-default `username` with the in-cluster PostgreSQL, it must contain <b>two</b> keys, not one: <ul>   <li>`postgresql-password` - the privileged user's password</li>   <li>`postgresql-postgres-password` - the built-in `postgres` superuser's password</li> </ul> The packaged PostgreSQL subchart references the second key without marking it optional, so if it is missing every database pod stops at `CreateContainerConfigError` and never starts. Magda generates both keys for you when `autoCreateSecret` is `true` and the secret is either absent or owned by the release; a pre-existing secret you manage is never modified. External databases (`useAwsRdsDb` / `useCloudSql`) do not use the second key. The two data keys are unchanged from v6 (`postgresql-password` and, for a non-default username, `postgresql-postgres-password`) — only the values path that names the secret moved under `auth`. |
| global.postgresql.auth.username | string | `"postgres"` | PostgreSQL username For in-k8s PostgreSQL, a user account will be auto-created with superuser privileges when username is `postgres`. It's recommended use superuser `postgres` for in-k8s PostgreSQL deployments. For cloud provider DB services (e.g. CloudSQL or AWS RDS), set this value to your external privileged DB account username. This user will only be used for DB schema migrators to cerate DB schema and restricted DB accounts that are used by Magda internal services to access DB. If you have to use a user account rather than `postgres`, the user account needs to have sufficient permissions to run all DB migration scripts ([e.g. here](https://github.com/magda-io/magda/tree/master/magda-migrator-registry-db/sql)). Note: Until the ticket #3126 is fixed, using a DB username rather than `postgres` will trigger an error when content DB migrate runs. <br/><br/> For the in-cluster PostgreSQL this value <b>cannot be changed after the first boot</b>. The `CREATEDB`/`CREATEROLE` grant that makes a non-default privileged user workable is applied by an initdb script, and initdb scripts run only once, against an empty data directory. Pointing an existing deployment at a new username therefore leaves that account without the privileges the DB migrators need, and the migrators will fail with `permission denied to create role`. Choose the username at install time, or grant the privileges by hand (`ALTER ROLE <name> CREATEDB CREATEROLE;`) after changing it. |
| global.postgresql.autoCreateSecret | bool | `true` | When `true`, the secret named by `auth.existingSecret` is auto-created whenever it is absent (or is owned by the current Helm release); a pre-existing secret you manage is left unchanged. Creation applies to <b>both</b> the in-cluster and external-DB (`useAwsRdsDb` / `useCloudSql`) cases — it is <b>not</b> gated on the database type. Only the password <i>value</i> differs: for an external DB it is copied from the legacy "cloudsql-db-credentials" secret's `password` field when that secret exists (backward compatibility); in every other case a random password is generated.<br/><br/> <b>Important for external databases:</b> if you do not pre-create this secret with your real master-account password, Helm auto-creates it with a <i>random</i> password that will not match your managed DB account — services and migrator Jobs then fail with a password-authentication error rather than an obvious missing-secret error. Pre-create it (see `existingSecret` above) when pointing Magda at an external database. |
| global.postgresql.client.sslRootCertSecret | object | `{"key":"ca.crt","name":""}` | Kubernetes Secret supplying the CA certificate used to verify the PostgreSQL server certificate. REQUIRED whenever `sslmode` is `verify-ca` or `verify-full` — the chart fails to render without it, even if your server's CA is a publicly-trusted root, because Magda's DB migrator images ship libpq < 16 and cannot use a system trust store. Mounted read-only at /etc/magda/postgresql-ca/root.crt. |
| global.postgresql.client.sslRootCertSecret.key | string | `"ca.crt"` | Key within the Secret whose value is the PEM CA bundle. |
| global.postgresql.client.sslRootCertSecret.name | string | `""` | Name of the Secret holding the CA bundle. Empty disables CA mounting. |
| global.postgresql.client.sslmode | string | `""` | The `sslmode` used by all Magda services, DB migrators and the backup job when connecting to PostgreSQL. Injected as the standard libpq `PGSSLMODE` environment variable, and as an `sslmode=` JDBC URL parameter for the JVM-based components. Exception: the external authentication plugins (magda-auth-*) do not yet read this value and connect to the session DB in plaintext regardless (a pre-existing, separately tracked gap) -- this value is not a blanket guarantee across every workload that talks to PostgreSQL. Supported values are `require` (encrypt, without verifying the server certificate), `disable` (plaintext), `verify-ca` and `verify-full` (verify the server certificate). `prefer` and `allow` are rejected: node-postgres cannot negotiate them consistently, so accepting them would give the Node services different semantics from every other component. `verify-ca` / `verify-full` REQUIRE `sslRootCertSecret.name` below — rendering fails without it, even for a publicly-trusted CA, because Magda's DB migrator images ship libpq < 16 and have no system trust-store fallback. Leave empty to auto-resolve: `disable` when `useCloudSql` is enabled (the cloud_sql_proxy listener is plaintext by design and performs TLS to Cloud SQL itself), `require` otherwise. Setting this to `disable` is the single-flag rollback to the previous (unenforced, plaintext) client behaviour. The in-cluster PostgreSQL server always serves TLS by default; its listener is controlled per DB chart by `<db-chart>.magda-postgres.postgresql.tls.enabled` (there is no `global` equivalent — see the `magda-postgres` chart README). Turning that listener off while this value still resolves to `require` is rejected at render time rather than failing at runtime. |
