...
router.get("/public/users/whoami", function (req, res) {
    GetUserId_1.getUserIdHandling(req, res, (userId) => handlePromise(res, db_1.getUser(userId)));
});
router.get("/public/users/:userId", (req, res) => {
    const userId = req.params.userId;
    const getPublicUser = db_1.getUser(userId).then(userMaybe => userMaybe.map(user => {
        const publicUser = {
            id: user.id,
