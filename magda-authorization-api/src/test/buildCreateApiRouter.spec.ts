    ...
    describe("POST /private/users", () => {
        silenceErrorLogs(()=>{
            ...
            it("should return 401 status code without creating a new user if called without sepecifying user ID", async ()=>{
                ...
                await jsc.assert(jsc.forall(userDataArb, async userData=>{
                    try{
                        ...
                        expect(mockUserDataStore.countRecord()).to.equal(currentMockUserStoreSize);
            ...
            it("should create a new user if called as an admin user", async()=>{
                ...
                await jsc.assert(jsc.forall(userDataArb, async userData=>{
                    ...
                    try{
                        ...
                        expect(res.body).to.deep.equal({...userData, id: res.body.id});

                        const users = mockUserDataStore.getRecordByUserId(newUserId);
                        expect(users).to.be.an("array").that.is.not.empty;
                        expect(users[0]).to.deep.equal({...userData, id: newUserId})
    ...
    describe("GET /private/users/lookup", () => {
        silenceErrorLogs(()=>{
            it("should return 401 status code if requested without sepecifying user ID", async ()=>{
                ...
                await jsc.assert(jsc.forall(jsc.elements(newlyCreatedUsers), async ({source, sourceId})=>{
                    try{
                        const req = request(app)
            ...
            it("should return previous created user data if queried by previous `source` & `sourceID` and requested as an admin user", async()=>{
                ...
                await jsc.assert(jsc.forall(jsc.elements(newlyCreatedUsers), async (userData)=>{
    ...
    describe("GET /private/users/:userId", () => {
        silenceErrorLogs(()=>{
            it("should return 401 status code if requested without sepecifying user ID", async ()=>{
                ...
                await jsc.assert(jsc.forall(jsc.elements(newlyCreatedUsers), async ({id : userId})=>{
                    try{
                        const req = request(app)
            ...
            it("should return previous created user data if queried by previous `userId` and requested as an admin user", async()=>{
                ...
                await jsc.assert(jsc.forall(jsc.elements(newlyCreatedUsers), async (userData)=>{
                    ...
                    try{
                        ...
                        const req = request(app).get(`/private/users/${userId}`);
        ...
        silenceErrorLogs(()=>{
            ...
            it("should return previous created user data if queried by previous `userId` and requested as an admin user", async()=>{
                ...
                await jsc.assert(jsc.forall(jsc.elements(newlyCreatedUsers), async (userData)=>{
                    ...
                    try{
                        const {id : userId} = userData;
                        const req = request(app).get(`/public/users/${userId}`);
    ...
    describe("GET /public/users/whoami", () => {
        silenceErrorLogs(()=>{
            it("should return 401 status code without including session header", async ()=>{
                ...
                await jsc.assert(jsc.forall(jsc.nat, async ()=>{
                    try{
                        const req = request(app)
                            .get("/public/users/whoami");
            ...
            it("should return correct user data specified by session header", async()=>{
                ...
                await jsc.assert(jsc.forall(jsc.elements(mockUserDataStore.getData()), async (userData)=>{
                    try{
                        const {id : userId} = userData;
                        const req = request(app)
                            .get(`/public/users/whoami`);
