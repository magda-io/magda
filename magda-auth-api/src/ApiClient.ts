...
export default class ApiClient {
    ...
    getUser(userId: string): Promise<Maybe<User>> {
        return this.handleGetResult(fetch(`${this.baseUrl}/private/users/${userId}`));
    }

    getUserPublic(userId: string): Promise<Maybe<User>> {
        return this.handleGetResult(fetch(`${this.baseUrl}/public/users/${userId}`));
    }

    lookupUser(source: string, sourceId: string): Promise<Maybe<User>> {
        return this.handleGetResult(fetch(`${this.baseUrl}/private/users/lookup?source=${source}&sourceId=${sourceId}`));
    }
