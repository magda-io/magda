...
class Crawler(val externalInterfaces: Seq[InterfaceConfig], registry: Registry)(implicit val system: ActorSystem, implicit val config: Config, implicit val materializer: Materializer) {
...
object Crawler {
  def apply(externalInterfaces: Seq[InterfaceConfig], registry: Registry)(implicit system: ActorSystem, config: Config, materializer: Materializer) =
    new Crawler(externalInterfaces, registry)
}