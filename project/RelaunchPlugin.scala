...
object RelaunchKeys {
  lazy val relaunch = taskKey[Boolean](
    "Launch this project using sbt-revolver whenever the incremental compile actually does something."
  )
}
...
object RelaunchPlugin extends AutoPlugin {
  override val projectSettings = Seq(
    RelaunchKeys.relaunch := Def.taskDyn {
      ...
      val isRunning = spray.revolver.Actions.revolverState
        .getProcess(thisProjectRef.value)
        .find(_.isRunning)
        .nonEmpty
      ...
      if (hasMainClass && (!isRunning || incrementalCompileDidSomething)) {
        Def.task {
          (spray.revolver.RevolverPlugin.autoImport.reStart in Compile)
            .toTask("")
            .value
          true
        }
