...
trait BaseApiSpec extends FunSpec with Matchers with ScalatestRouteTest with MagdaElasticSugar with BeforeAndAfterEach with BeforeAndAfterAll with MagdaGeneratorTest with MockServer {
  ...
  override def afterEach(): Unit = {
    ...
    println(s"         Indexing ${Math.round(totalNumOfDatasets*10000/Math.max(1, totalIndexingTime))} datasets per 10 seconds.")
  }
...
object BaseApiSpec {
  val testStates: List[(RegionSource, JsObject)] = {
    ...
    List(
      ...
      """.parseJson.asJsObject),
      (new RegionSource("ithinkthisisanotherregiontype",new URL("http://example.com"), "SA2_MAIN11", "SA2_NAME11", None, false, false, 10), """
