...
abstract class FacetSpecBase extends BaseSearchApiSpec {
  var defaultGen: Gen[((String, List[DataSet], Route), (String, Query), Seq[Nothing])]
  ...
  def genericFacetSpecs(facetType: FacetType, reducer: DataSet ⇒ Set[String], queryCounter: Query ⇒ Int, filterQueryGen: List[DataSet] => Gen[Query], specificGen: List[DataSet] => Gen[Query]): Unit = {
