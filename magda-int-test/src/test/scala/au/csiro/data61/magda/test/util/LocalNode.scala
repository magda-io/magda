...
trait LocalNode {
  ...
  def host: String
  def port: Int
  def client(shutdownNodeOnClose: Boolean): HttpClient
...
class RemoteLocalNode(val clusterName: String,
  ...
  override def client(shutdownNodeOnClose: Boolean): HttpClient =
...
class InternalLocalNode(settings: Settings, plugins: List[Class[_ <: Plugin]])
  ...
  /**
    ...
    */
  override def client(shutdownNodeOnClose: Boolean): HttpClient = new HttpClient {
    private val delegate            = HttpClient(s"elasticsearch://$host:$port")
    override def client = delegate.client
