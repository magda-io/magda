...
class ElasticSearchQueryer(indices: Indices = DefaultIndices)(
  ...
  val DATASETS_LANGUAGE_FIELDS = Seq(("title", 50f),
                                     ("description", 2f),
                                     "publisher.name",
                                     ("keywords", 10f),
                                     "themes")
  val NON_LANGUAGE_FIELDS = Seq("_id",
                                "catalog",
                                "accrualPeriodicity",
                                "contactPoint.name",
                                "publisher.acronym")

  override def search(inputQuery: Query,
                      start: Long,
                      limit: Int,
    ...
    clientFuture.flatMap { implicit client =>
      ...
      val query = buildQueryWithAggregations(inputQuery,
                                             start,
                                             limit,
                                             MatchAll,
                                             requestedFacetSize)
          ...
          client.execute(query).flatMap {
            case Right(results) =>
              if (results.result.totalHits > 0)
                ...
                client
                  .execute(
                    buildQueryWithAggregations(inputQuery,
                                               start,
                                               limit,
                                               MatchPart,
                                               requestedFacetSize))
      ...
      case RootCause(illegalArgument: IllegalArgumentException) =>
        ...
        failureSearchResult(inputQuery,
  ...
  /**
    ...
    */
  private def idealFacetSize(facetType: FacetType,
                             query: Query,
                             requestedFacetSize: Int) = {
    val queryFacetSize =
  ...
  /**
    ...
    */
  def buildSearchResult(query: Query,
      ...
      facets = Some(FacetType.all.map { facetType =>
        ...
        new Facet(
          ...
          options = {
            ...
            definition.truncateFacets(query,
                                      filteredOptions,
                                      exactOptions,
                                      alternativeOptions,
                                      facetSize)
  ...
  def buildQueryWithAggregations(query: Query,
                                 start: Long,
                                 limit: Int,
                                 strategy: SearchStrategy,
                                 facetSize: Int) =
    addAggregations(buildQuery(query, start, limit, strategy),
                    query,
                    strategy,
                    facetSize)
  ...
  def failureSearchResult(query: Query, message: String) =
    new SearchResult(query = query,
                     hitCount = 0,
                     dataSets = Nil,
                     errorMessage = Some(message))

  /** Adds standard aggregations to an elasticsearch query */
  def addAggregations(searchDef: SearchDefinition,
                      query: Query,
                      strategy: SearchStrategy,
                      facetSize: Int) = {
  ...
  def aggsForFacetType(query: Query,
                       facetType: FacetType,
                       strategy: SearchStrategy,
    ...
    val globalAgg =
      globalAggregation(facetType.id + "-global")
        .subAggregations(alternativesAggregation(
          ...
          facetDef,
          strategy,
          facetSize) :: exactMatchAggregations(query,
                                               facetType,
                                               facetDef,
                                               strategy))
  ...
  /**
    ...
    */
  def alternativesAggregation(query: Query,
                              facetDef: FacetDefinition,
                              strategy: SearchStrategy,
                              facetSize: Int) =
  ...
  private def buildEsQuery(query: Query,
  ...
  override def searchFacets(facetType: FacetType,
      ...
      client
        .execute(
          ElasticDsl
            ...
            .query(dismax(
              Seq(matchPhrasePrefixQuery("value", facetQuery.getOrElse("")),
          ...
          case Right(results) =>
            results.result.totalHits match {
              ...
              case _ =>
                ...
                client.execute {
                  buildQuery(facetDef.removeFromQuery(generalQuery),
                             0,
                             0,
                  ...
                  case Right(results) =>
                    val aggregations = results.result.aggregations.data.map {
                      case (name: String, value: Map[String, Any]) =>
                        (name,
                         new FacetOption(identifier = None,
                                         value = name,
                                         hitCount = value
                                           .get("doc_count")
                    ...
                    FacetSearchResult(hitCount = results.result.totalHits,
                                      options = options)
                }
            }
        }
  ...
  override def searchRegions(query: Option[String],
                             start: Long,
      ...
      client
        .execute(
          ElasticDsl.search(indices.getIndex(config, Indices.RegionsIndex))
            query {
              boolQuery().should(matchPhrasePrefixQuery(
                                   "regionShortName",
                                   query.getOrElse("*")).boost(2),
                                 matchPhrasePrefixQuery("regionName",
                                                        query.getOrElse("*")))
            }
            start start.toInt
            limit limit
    ...
    clientFuture.flatMap { client =>
      ...
      client
        ...
        .recover {
          ...
          case e: Throwable =>
            logger.error(e, "Exception when searching")
            OrganisationsSearchResult(queryString,
                                      0,
                                      List(),
  ...
  def resolveFullRegion(queryRegionFV: FilterValue[Region])(
      implicit client: HttpClient): Future[Option[Region]] = {
    queryRegionFV match {
