...
export default class ChartConfig extends Component {
  ...
  renderIconSelect(options, id, label){
      return (<div className='mui-textfield chart-config_icon-select'><label tabIndex="-1">{label}</label>{Object.keys(options).map(v=> (<button className={this.props.chartType === v ? 'isActive' : ''} onClick ={this.props.onChange.bind(this, id, v)} key={v} title={v}><img alt={v} src={VEGAMARK[v]}/></button>))}</div>)
  }
  ...
  render(){
    return (<div className='chart-config'>
              <div className='chart-type'>{this.renderIconSelect(VEGAMARK, 'chartType', 'Chart type')}</div>
              <div className='chart-title'><Input onChange = {this.onChange.bind(this, 'chartTitle')} label="Chart title" placeholder='Enter a descriptive chart title'/></div>
              <div className='y-axis'>{this.renderDropdownSelect(this.props.xAxisOptions, 'xAxis', 'xAxis')}</div>
              <div className='x-axis'>{this.renderDropdownSelect(this.props.yAxisOptions, 'yAxis', 'yAxis')}</div>
              <div className='linear'>{this.renderDropdownSelect(DATATYPE, 'yScale', 'Chart scale')}</div>
            </div>)
  }
}
