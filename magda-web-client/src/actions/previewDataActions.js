...
export function fetchPreviewData(distribution) {
    return (dispatch: Function, getState: Function) => {
        ...
        if (!prop) {
            return dispatch(
                requestPreviewDataError(
                    "No preview available because the url to access the data is broken"
                )
            );
        }
        ...
        if (
            ...
            prop.url === getState().previewData.url
        ) {
            return false;
        }
            ...
            case "geo":
                ...
                let geoData = {
                    data:
                        ...
                        "#start=" +
                        encodeURIComponent(JSON.stringify(catalog)),
                    meta: {
                        type: "geo"
                    }
                };
                dispatch(
                    receivePreviewData({ [distribution.identifier]: geoData })
                );
                break;
