...
const temporalExtent = jsonpath.query(extent, '$[*].temporalElement[*].EX_TemporalExtent[*].extent[*]')

const beginPosition = jsonpath.query(temporalExtent, '$[*].TimePeriod[*].beginPosition[*]');
const endPosition = jsonpath.query(temporalExtent, '$[*].TimePeriod[*].endPosition[*]');
const beginTimePosition = jsonpath.query(temporalExtent, '$[*].TimePeriod[*].begin[*].TimeInstant[*].timePosition[*]');
const endTimePosition = jsonpath.query(temporalExtent, '$[*].TimePeriod[*].end[*].TimeInstant[*].timePosition[*]');
...
const beginIndeterminate = jsonpath.value(allBegin, '$[*]["$"].indeterminatePosition.value');

let endValue = jsonpath.value(allEnd, '$[*]._');
const endIndeterminate = jsonpath.value(allEnd, '$[*]["$"].indeterminatePosition.value');

if ((!beginValue && beginIndeterminate === 'now') || (!endValue && endIndeterminate === 'now')) {
