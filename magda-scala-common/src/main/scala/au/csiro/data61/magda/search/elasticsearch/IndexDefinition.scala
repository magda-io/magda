...
object IndexDefinition extends DefaultJsonProtocol {
  ...
  def magdaAutocompleteField(name: String) = {
    textField(name).analyzer("magda_edge_ngram").searchAnalyzer("standard")
  }
        ...
        .mappings(
          mapping(indices.getType(Indices.DataSetsIndexType)).fields(
            ...
            objectField("accessNotes").fields(
              keywordField("url"),
              magdaTextField("notes").fields(magdaAutocompleteField("autocomplete")),
    ...
    loader.setupRegions
      .map {
        case (regionSource, jsonRegion) =>
          ...
          geometryOpt.map(
            geometry =>
              indexInto(indices.getIndex(config, Indices.RegionsIndex) / indices
      ...
      .runWith(Sink.reduce((oldLength: AnyVal, latestValuesLength: AnyVal) =>
