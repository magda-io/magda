...
object Registry {
  ...
  case class AspectDefinition(
      ...
      @(ApiModelProperty @field)(value = "The name of the aspect.",
                                 required = true) name: String,
      @(ApiModelProperty @field)(
        value = "The JSON Schema of this aspect.",
        required = false,
        dataType = "object") jsonSchema: Option[JsObject])

  case class WebHookPayload(action: String,
                            lastEventId: Long,
  ...
  case class RegistryEvent(id: Option[Long],
  ...
  case class Record(@(ApiModelProperty @field)(
                      value = "The unique identifier of the record",
                      required = true) id: String,
                    @(ApiModelProperty @field)(value = "The name of the record",
                                               required = true) name: String,
                    @(ApiModelProperty @field)(
                      value = "The aspects included in this record",
                      required = true,
                      dataType = "object") aspects: Map[String, JsObject],
                    @(ApiModelProperty @field)(
  ...
  case class RecordSummary(
      @(ApiModelProperty @field)(value = "The unique identifier of the record",
                                 required = true) id: String,
      @(ApiModelProperty @field)(value = "The name of the record",
                                 required = true) name: String,
      @(ApiModelProperty @field)(
        value = "The list of aspect IDs for which this record has data",
        required = true) aspects: List[String])
      extends RecordType
  ...
  case class WebHook(id: Option[String] = None,
                     userId: Option[Int],
                     name: String,
                     active: Boolean,
  ...
  case class WebHookConfig(aspects: Option[List[String]] = None,
  ...
  case class RegistryRecordsResponse(hasMore: Boolean,
  ...
  case class WebHookAcknowledgement(@(ApiModelProperty @field)(
                                      value =
  ...
  case class WebHookAcknowledgementResponse(
      @(ApiModelProperty @field)(
        ...
        required = true) lastEventIdReceived: Long)

  trait RegistryProtocols
  ...
  trait RegistryConverters extends RegistryProtocols {
    ...
    def getAcronymFromPublisherName(
        publisherName: Option[String]): Option[String] = {
      publisherName
    ...
    def getNullableStringField(data: JsObject,
      ...
      data.fields.get(field) match {
        ...
        case _ =>
          deserializationError(s"Invalid nullableString field: ${field}")
      }
    }

    def convertRegistryDataSet(hit: Record,
                               logger: Option[LoggingAdapter] = None)(
        implicit defaultOffset: ZoneOffset): DataSet = {
      val dcatStrings =
        hit.aspects.getOrElse("dcat-dataset-strings", JsObject())
      ...
      val coverageStart = ApiDate(
        tryParseDate(
          Try[Option[String]] {
            temporalCoverage.extract[String](
              'intervals.? / element(0) / 'start.?)
          } match {
      ...
      DataSet(
        ...
        contactPoint = dcatStrings
          .extract[String]('contactPoint.?)
          .map(cp => Agent(Some(cp))),
        ...
        publishingState = Some(
          publishing
            .extract[String]('state.?)
            .getOrElse("published")), // assume not set means published
        accessControl = accessControl
      )
    }

    private def convertDistribution(distribution: JsObject, hit: Record)(
        implicit defaultOffset: ZoneOffset): Distribution = {
    ...
    private def tryParseDate(dateString: Option[String])(
