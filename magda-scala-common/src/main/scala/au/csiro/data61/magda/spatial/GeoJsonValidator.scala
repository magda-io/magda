...
object GeoJsonValidator {
  ...
  def validateCoordinate(c:GeoJson.Coordinate): Boolean = {
    if(c.x.abs > 180.toDouble) throw new Exception(s"Invalid Longitude (x) range: ${c.x}. Should be within -180 to +180")
    if(c.y.abs > 90.toDouble) throw new Exception(s"Invalid latitude (y) range: ${c.y}. Should be within -90 to +90")
    true
  }
