...
trait MagdaGeneratorTest extends GeneratorDrivenPropertyChecks {
  ...
  implicit override val generatorDrivenConfig: PropertyCheckConfiguration =
    PropertyCheckConfiguration(workers = PosInt.from(processors).get, sizeRange = if (ContinuousIntegration.isCi) 25 else 25, minSuccessful = PosInt.from(minSuccessful).get)
}