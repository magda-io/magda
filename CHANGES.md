...
- #3711: Fix semantic search being unavailable on deployments built with Helm ≤ 3.16. Older Helm doesn't honour the magda chart's `encoder: null` override when coalescing `global` values, so `magda-{csv,pdf}-semantic-indexer` rendered an invalid config with both `compressionLevel` and `encoder` set, crash-looping the semantic indexers so the semantic index was never created. Bump the Node.js builder image's pinned Helm from 3.13.2 to 3.17.4, add a `magda-core` template guard that fails fast on Helm < 3.17.0, and consume `magda-csv-semantic-indexer` 1.0.0-alpha.2 (whose default no longer sets a populated `encoder`, so the config is correct regardless of Helm version).
