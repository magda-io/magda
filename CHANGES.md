...
- #3749: Upgrade the bundled in-cluster PostgreSQL from 13.7 to **17.5** (bitnami `postgresql` subchart 10.9.1 → 16.7.24). PostgreSQL 13 reached end of life in November 2025. **Breaking — read before upgrading:**
  - Three `global.postgresql.*` values moved: `postgresqlUsername` → `auth.username`, `postgresqlDatabase` → `auth.database`, `existingSecret` → `auth.existingSecret`. The chart now **fails at render time** if the old paths are set, rather than silently ignoring them. The `db-main-account-secret` data keys are unchanged (`postgresql-password`, and `postgresql-postgres-password` for a non-default username), so no secret needs rewriting.
  - Per-DB-chart values moved under `primary.`: `persistence`, `resources`, `livenessProbe`, `readinessProbe`. `tls.certificatesSecret` no longer needs setting per chart — it is derived from `fullnameOverride`.
  - The in-cluster instance is renamed `<db>-postgresql` → `<db>-postgresql-pg17`: new StatefulSet, new data PVCs, new TLS certificate secret. Client-facing Service names (`registry-db`, `content-db`, `session-db`, `tenant-db`, `authorization-db`) are unchanged, so nothing in application config moves. `combined-db` has no separate alias Service — clients connect directly to the renamed `combined-db-postgresql-pg17` Service, so that name changes too.
  - The root `volumePermissions` init container is gone; the certificate is staged by the non-privileged `copy-certs` init container instead, which satisfies the OpenShift restricted SCC.
  - **A v6 → v7 `helm upgrade` cannot upgrade the database in place** — PostgreSQL 13 data files are not readable by PostgreSQL 17, and chart 16.7.24 adds `app.kubernetes.io/component: primary` to the StatefulSet's immutable `spec.selector`, so an in-place upgrade across the subchart major is impossible. That is why the instance is renamed to `<db>-postgresql-pg17`: it is a **new, empty** StatefulSet. Data in the old PostgreSQL 13 instance is **not** carried over automatically, and its PVC (`data-<db>-postgresql-0`) is left untouched, unless the automated migration path described below (#3750) is explicitly opted into. The wal-g backup chain taken on v6 restores only into PostgreSQL 13 and is rollback material, not a migration path.
  - `global.security.allowInsecureImages: true` is now set unconditionally. Bitnami chart 16.x hard-fails rendering (`common.errors.insecureImages`) whenever any image is not the stock upstream one, and every Magda image is served from `ghcr.io/magda-io/...`, never `docker.io/bitnami` (whose tags now resolve only from the frozen, unmaintained `bitnamilegacy/*` namespace). This value is release-wide — Helm merges every subchart's `global` block into one shared namespace — not scoped to the `postgresql` subchart, so it also applies if you set your own `global.security.*` values.
  - The bundled wal-g is now a **custom Magda build**, `ghcr.io/magda-io/magda-wal-g:3.0.8-magda-edcda8b`, not an upstream release. PostgreSQL 15 changed the `BASE_BACKUP` replication protocol, and every upstream wal-g release (1.1.0 through 3.0.8) fails `backup-push` against PostgreSQL 15+ in the remote mode Magda's backup CronJob uses ([wal-g/wal-g#2044](https://github.com/wal-g/wal-g/issues/2044), [#1385](https://github.com/wal-g/wal-g/issues/1385), [#2005](https://github.com/wal-g/wal-g/issues/2005)). The fix ([wal-g/wal-g#2262](https://github.com/wal-g/wal-g/pull/2262)) merged upstream four months after the newest release and is in no released version yet, so Magda builds from a [fork](https://github.com/magda-io/wal-g/releases/tag/v3.0.8-magda-edcda8b) carrying it. This is temporary and should be reverted to an official `wal-g/wal-g` release once one includes PR #2262.
  - **Delta (incremental) backups remain unavailable** — they need filesystem access to compute changed blocks, which only wal-g's local backup mode has, not the remote mode above. Evaluated and declined; tracked as [magda-io/magda#3761](https://github.com/magda-io/magda/issues/3761) (closed as not planned).
  - Every `helm template`/`helm install` now prints two `coalesce.go:316: warning: cannot overwrite table with non table` lines, because `primary.customLivenessProbe`/`customReadinessProbe` are set as plain strings (they embed subchart `{{ }}` helpers) where the subchart schema declares them as maps. This is expected and harmless — the string still wins and renders correctly — not a bug report.
- #3750: Add an automated in-cluster PostgreSQL major-upgrade path for v6 → v7. Setting `<db>.magda-postgres.majorUpgrade.enabled: true` (via the umbrella chart that is `magda-core.combined-db.magda-postgres.majorUpgrade.enabled: true`; Helm accepts an unknown `--set` path silently, so an unprefixed path skips the migration and the new instance comes up empty) makes a `pre-upgrade` hook Job take a `pg_dumpall` of the running PostgreSQL 13 instance to a staging volume, and a `post-upgrade` hook Job load it into the new PostgreSQL 17 instance before the DB migrators run. **Requires `global.postgresql.auth.username: postgres`** (the default) — the restore Job fails fast with an actionable message otherwise, because `pg_dumpall --clean --if-exists` emits a `DROP ROLE IF EXISTS postgres;` for the bootstrap role that no other `${PGUSER}`-scoped filter covers. **Also requires an explicit, generous `helm upgrade --timeout`** — Helm's default (5 minutes) is too short for a real dump + restore, and it is Helm's own `--timeout`, not `majorUpgrade.waitTimeoutSeconds`, that aborts the upgrade if exceeded. The old data PVC is never written to, so `helm rollback` returns to a working PostgreSQL 13. See the [PostgreSQL major upgrade runbook](./docs/docs/postgres-major-upgrade-runbook.md). Note that wal-g backups do not cross major versions: the v6 chain restores only into PostgreSQL 13, so take a fresh base backup after cutting over.
...
- #3747: Add a wal-g backup/restore integration test harness (`magda-int-test-ts`) as a regression oracle for the in-cluster PostgreSQL modernisation baseline. Adds in-cluster DB backup/restore mechanics & RPO documentation.
- #3754: Make in-cluster PostgreSQL recovery target configurable via `backupRestore.recoveryMode.recoveryTarget` (roll-forward / point-in-time / base-backup). **Behaviour change:** the default recovery now **rolls forward to the latest archived WAL** (`"latest"`) instead of stopping at the base backup (`"immediate"`); set `recoveryTarget: "immediate"` to keep the previous behaviour.
