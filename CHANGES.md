...
- Upgraded the in-cluster PostgreSQL backup tool **wal-g 1.1.0 → 3.0.8** (`magda-postgres`). This is config-compatible: the S3/storage env vars, `storageConfig` contract, and backup/restore commands are unchanged. <!-- COMPAT-CLAUSE: filled from cross-version E2E (Phase C) — either "Existing 1.1.0 backups restore under 3.0.8." or "Take a fresh base backup after upgrading." --> (#3758)
