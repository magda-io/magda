...
- Upgraded the in-cluster PostgreSQL backup tool **wal-g 1.1.0 → 3.0.8** (`magda-postgres`). This is config-compatible: the S3/storage env vars, `storageConfig` contract, and backup/restore commands are unchanged. Existing wal-g 1.1.0 backups remain restorable under 3.0.8 (verified by the integration test harness), so no re-backup is required on upgrade. (#3758)
