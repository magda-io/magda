...
Before upgrade to `0.0.57`, run `helm repo list` to make sure the repo `magda-io` (url: https://charts.magda.io) and the repo stable (url: https://kubernetes-charts.storage.googleapis.com) are included. If not, run
