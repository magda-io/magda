          ...
          (sourceTagToPreserve, sourceId) =>
            ...
            deleteResult match {
              ...
              case Failure(exception) =>
                ...
                complete(
                  StatusCodes.BadRequest,
                  BadRequest("An error occurred while processing your request.")
                )
            }
    ...
    path(Segment) { (id: String) =>
      {
        entity(as[Record]) { record: Record =>
          withAspectOpaQuery(
            ...
            val result = DB localTx { session =>
              recordPersistence
                .putRecordById(session, id, record, opaQueries) match {
                case Success(_) =>
            ...
            webHookActor ! WebHookActor
              .Process(false, Some(record.aspects.map(_._1).toList))
            result
          }
        }
      }
