...
class RecordHistoryServiceSpec extends ApiSpec {
  ...
  describe("History Api") {
    ...
    def installAspectSchema(aspectId: String)(
      ...
      param.asAdmin(Post(s"/v0/aspects", aspectDef)) ~> addTenantIdHeader(
        TENANT_1) ~> param.api(Full).routes ~> check {
        status shouldBe StatusCodes.OK
      }
    }

    def createAspects()(implicit param: FixtureParam): Unit = {
      val requiredAspects = List("dcat-dataset-strings",
    ...
    def createDataset(recordIdSuffix: String = "")(
      ...
      val orgRecord = Record(
        ...
        Map(
          "organization-details" -> JsObject(
            Map("name" -> JsString(s"test org ${recordIdSuffix}"),
      ...
      val distRecord = Record(
        ...
        Map(
          "dcat-distribution-strings" -> JsObject(
            Map("title" -> JsString(s"test distribution ${recordIdSuffix}"),
    ...
    def getEvents(recordId: String,
      ...
      Get(s"/v0/records/${recordId}/history${if (queryParams.isEmpty) ""
      else "?" + queryParams.mkString("&")}") ~> addTenantIdHeader(TENANT_1) ~> param
        .api(ReadOnly)
        .routes ~> check {
    ...
    def selectEvents(events: Seq[RegistryEvent],
                     recordIds: Seq[String] = Seq(),
                     aspectIds: Seq[String] = Seq()) = events.filter { event =>
    ...
    describe("Get History By Record") {
      it("should only return events for selected record when dereference is false") {
        implicit param =>
          createAspects()
        ...
        implicit param =>
          ...
          selectEvents(events,
                       recordIds = List(
                         records1._1.id,
                         records1._2.id,
                         records1._3.id)).size shouldEqual events.size
          ...
          selectEvents(
            ...
            aspectIds = List("dcat-distribution-strings")).size should be > 0
          selectEvents(
            events,
            recordIds = List(records1._2.id),
            aspectIds = List("organization-details")).size should be > 0
      ...
      it("should also include events from removed distributions when dereference is true") {
        implicit param =>
          createAspects()
          val records1 = createDataset("1")
          val records2 = createDataset("2")
      ...
      it("should also include events from newly assigned publisher when dereference is true") {
        implicit param =>
          createAspects()
          val records1 = createDataset("1")
          val records2 = createDataset("2")
          ...
          param.asAdmin(Put(
            ...
            JsObject(Map("publisher" -> JsString(
