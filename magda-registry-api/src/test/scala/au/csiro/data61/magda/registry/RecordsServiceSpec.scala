  ...
  def readOnlyTests(role: Role) {
    describe("GET") {
      it("starts with no records defined") { param =>
        Get("/v0/records") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
      ...
      it("returns 404 if the given ID does not exist") { param =>
        Get("/v0/records/foo") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
      ...
      it("returns 404 if the given ID does not have a required aspect") { param =>
        ...
        Get("/v0/records/foo?aspect=test") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.NotFound
          responseAs[BadRequest].message should include("exist")
        }
      }
      ...
      describe("summary") {
        it("/records/summary/{id} returns a summary with id, name, and aspect ids for which the record has data in the aspect") { param =>
          ...
          param.asAdmin(Post("/v0/records", recordWithAspects)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records/summary/id") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
        ...
        it("/records/summary returns a list of summaries with id, name, and aspect ids for which the record has data in the aspect") { param =>
          ...
          param.asAdmin(Post("/v0/records", recordWithAspects1)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val recordWithAspects2 = Record("id2", "name2", Map("test2" -> JsObject(), "test3" -> JsObject()))
          param.asAdmin(Post("/v0/records", recordWithAspects2)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val recordWithoutAspects = Record("id3", "name3", Map())
          param.asAdmin(Post("/v0/records", recordWithoutAspects)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records/summary") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
        ...
        def insertAspectDefs(param: FixtureParam) {
          ...
          param.asAdmin(Post("/v0/aspects", aspectDefinition1)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }
          ...
          param.asAdmin(Post("/v0/aspects", aspectDefinition3)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }
        }
      }
      ...
      describe("count") {
        it("returns the right count when no parameters are given") { param =>
          ...
          Get("/v0/records/count") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
      ...
      describe("aspects") {
        it("includes optionalAspect if it exists") { param =>
          ...
          param.asAdmin(Post("/v0/records", recordWithAspect)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val recordWithoutAspect = Record("without", "without", Map())
          param.asAdmin(Post("/v0/records", recordWithoutAspect)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records?optionalAspect=test") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 2
            page.records.head shouldBe recordWithAspect.copy(tenantId = Some(SINGLE_TENANT_ID))
            page.records(1) shouldBe recordWithoutAspect.copy(tenantId = Some(SINGLE_TENANT_ID))
          }
        }
        ...
        it("requires presence of aspect") { param =>
          ...
          Get("/v0/records?aspect=test") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
          ...
          Get("/v0/records/count?aspect=test") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
        ...
        it("requires any specified aspects to be present") { param =>
          ...
          param.asAdmin(Post("/v0/aspects", fooAspect)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val barAspect = AspectDefinition("bar", "bar", None)
          param.asAdmin(Post("/v0/aspects", barAspect)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val withFoo = Record("withFoo", "with foo", Map("foo" -> JsObject("test" -> JsString("test"))))
          param.asAdmin(Post("/v0/records", withFoo)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val withBar = Record("withBar", "with bar", Map("bar" -> JsObject("test" -> JsString("test"))))
          param.asAdmin(Post("/v0/records", withBar)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val withFooAndBar = Record("withFooAndBar", "with foo and bar", Map("foo" -> JsObject(), "bar" -> JsObject("test" -> JsString("test"))))
          param.asAdmin(Post("/v0/records", withFooAndBar)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records?aspect=foo&aspect=bar") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 1
            page.records.head shouldBe withFooAndBar.copy(tenantId = Some(SINGLE_TENANT_ID))
          }

          Get("/v0/records/count?aspect=foo&aspect=bar") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
            countResponse.count shouldBe 1
          }
        ...
        it("optionalAspects are optional") { param =>
          ...
          param.asAdmin(Post("/v0/records", withNone)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records?optionalAspect=foo&optionalAspect=bar") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 4
            page.records.head shouldBe withFoo.copy(tenantId = Some(SINGLE_TENANT_ID))
            page.records(1) shouldBe withBar.copy(tenantId = Some(SINGLE_TENANT_ID))
            page.records(2) shouldBe withFooAndBar.copy(tenantId = Some(SINGLE_TENANT_ID))
            page.records(3) shouldBe withNone.copy(tenantId = Some(SINGLE_TENANT_ID))
          }
        }
        ...
        it("supports a mix of aspects and optionalAspects") { param =>
          ...
          param.asAdmin(Post("/v0/aspects", bazAspect)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }
          ...
          param.asAdmin(Post("/v0/records", withFooAndBarAndBaz)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }
          ...
          Get("/v0/records?aspect=foo&optionalAspect=bar") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 2
            page.records.head shouldBe withFoo.copy(tenantId = Some(SINGLE_TENANT_ID))
            ...
            page.records(1) shouldBe withFooAndBarAndBaz.copy(aspects = withFooAndBarAndBaz.aspects - "baz", tenantId = Some(SINGLE_TENANT_ID))
          }
        }

        it("accepts URL-encoded aspect names") { param =>
          val aspect = AspectDefinition("with space", "foo", None)
          param.asAdmin(Post("/v0/aspects", aspect)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }
          ...
          Get("/v0/records?optionalAspect=with%20space") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 1
            page.records.head shouldBe record.copy(tenantId = Some(SINGLE_TENANT_ID))
          }

          Get("/v0/records?aspect=with%20space") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 1
            page.records.head shouldBe record.copy(tenantId = Some(SINGLE_TENANT_ID))
          ...
          Get("/v0/records/count?aspect=with%20space") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
            countResponse.count shouldBe 1
          }
      ...
      describe("dereference") {
        it("dereferences a single link if requested") { param =>
          ...
          param.asAdmin(Post("/v0/records", source)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val target = Record("target", "target", Map("withLink" -> JsObject("someLink" -> JsString("source"))))
          param.asAdmin(Post("/v0/records", target)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records/source?aspect=withLink") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            responseAs[Record].aspects("withLink") shouldBe JsObject(
              "someLink" -> JsString("target"))
          }

          Get("/v0/records/source?aspect=withLink&dereference=false") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            responseAs[Record].aspects("withLink") shouldBe JsObject(
              "someLink" -> JsString("target"))
          }

          Get("/v0/records/source?aspect=withLink&dereference=true") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            responseAs[Record].aspects("withLink") shouldBe JsObject(
              "someLink" -> JsObject(
                "id" -> JsString("target"),
        ...
        it("dereferences an array of links if requested") { param =>
          ...
          param.asAdmin(Post("/v0/records", anotherTarget)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records/source?aspect=withLinks") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            responseAs[Record].aspects("withLinks") shouldBe JsObject(
              "someLinks" -> JsArray(JsString("target"), JsString("anotherTarget")))
          }

          Get("/v0/records/source?aspect=withLinks&dereference=false") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            responseAs[Record].aspects("withLinks") shouldBe JsObject(
              "someLinks" -> JsArray(JsString("target"), JsString("anotherTarget")))
          }

          Get("/v0/records/source?aspect=withLinks&dereference=true") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            responseAs[Record].aspects("withLinks") shouldBe JsObject(
              "someLinks" -> JsArray(
                JsObject(
      ...
      describe("querying by aspect value") {
        it("works for shallow paths") { param =>
          ...
          param.asAdmin(Post("/v0/records", recordWithValue1)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val recordWithValue2 = Record("withValue2", "withValue2", Map("exampleAspect" -> JsObject("value" -> JsString("correct"))))
          param.asAdmin(Post("/v0/records", recordWithValue2)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          val recordWithoutValue = Record("withoutValue", "withoutValue", Map("exampleAspect" -> JsObject("value" -> JsString("incorrect"))))
          param.asAdmin(Post("/v0/records", recordWithoutValue)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }

          Get("/v0/records?aspectQuery=exampleAspect.value:correct&aspect=exampleAspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 2
            page.records.head shouldBe recordWithValue1.copy(tenantId = Some(SINGLE_TENANT_ID))
            page.records(1) shouldBe recordWithValue2.copy(tenantId = Some(SINGLE_TENANT_ID))
          }

          Get("/v0/records/count?aspectQuery=exampleAspect.value:correct&aspect=exampleAspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
        ...
        it("works without specifying the aspect in aspects or optionalAspects") { param =>
          ...
          Get("/v0/records?aspectQuery=exampleAspect.value:correct") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 2
          ...
          Get("/v0/records/count?aspectQuery=exampleAspect.value:correct") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
            countResponse.count shouldBe 2
          }
        ...
        it("works for deep paths") { param =>
          ...
          Get("/v0/records?aspectQuery=exampleAspect.object.value:correct&aspect=exampleAspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 2
            page.records.head shouldBe recordWithValue1.copy(tenantId = Some(SINGLE_TENANT_ID))
          ...
          Get("/v0/records/count?aspectQuery=exampleAspect.object.value:correct&aspect=exampleAspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
            countResponse.count shouldBe 2
          }
        ...
        it("works as AND when multiple queries specified") { param =>
          ...
          Get("/v0/records?aspectQuery=exampleAspect.value:correct&aspectQuery=exampleAspect.otherValue:alsoCorrect&aspect=exampleAspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 2
            page.records.head shouldBe recordWithValue1.copy(tenantId = Some(SINGLE_TENANT_ID))
          ...
          Get("/v0/records/count?aspectQuery=exampleAspect.value:correct&aspectQuery=exampleAspect.otherValue:alsoCorrect&aspect=exampleAspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
            countResponse.count shouldBe 2
          }
        ...
        it("allows url encoded paths and values") { param =>
          ...
          param.asAdmin(Post("/v0/records", recordWithValue)) ~> addSingleTenantIdHeader ~> param.api(Full).routes ~> check {
            status shouldEqual StatusCodes.OK
          }
          ...
          Get("/v0/records?aspectQuery=example%20Aspect.%26value:%2Fcorrect&aspect=example%20Aspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val page = responseAs[RecordsPage[Record]]
            page.records.length shouldBe 1
            page.records.head shouldBe recordWithValue.copy(tenantId = Some(SINGLE_TENANT_ID))
          }

          Get("/v0/records/count?aspectQuery=example%20Aspect.%26value:%2Fcorrect&aspect=example%20Aspect") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            val countResponse = responseAs[CountResponse]
            countResponse.count shouldBe 1
          }
      ...
      describe("paging") {
        ...
        def pagingTests(path: String) {
          it("honors the limit parameter") { param =>
            ...
            Get(s"/v0/records${path}limit=2") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
          ...
          it("honors the start parameter") { param =>
            ...
            Get(s"/v0/records${path}start=3&limit=2") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
              page.records.length shouldBe 2
          ...
          it("pageTokens can be used to page through results") { param =>
            ...
            var currentPage = Get(s"/v0/records${path}limit=2") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
              page.records.length shouldBe 2
              page.records.head.name shouldBe "1"
            ...
            currentPage =
              Get(s"/v0/records${path}pageToken=${currentPage.nextPageToken.get}&limit=2") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
                status shouldEqual StatusCodes.OK
                val page = responseAs[RecordsPage[RecordType]]
                page.records.length shouldBe 2
            ...
            currentPage = Get(s"/v0/records${path}pageToken=${currentPage.nextPageToken.get}&limit=2") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
              page.records.length shouldBe 1
          ...
          it("provides hasMore correctly") { param =>
            ...
            Get(s"/v0/records${path}start=0&limit=4") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
            ...
            Get(s"/v0/records${path}start=0&limit=5") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
            ...
            Get(s"/v0/records${path}start=0&limit=6") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
              page.hasMore shouldBe false
              page.nextPageToken.isDefined shouldBe false
            ...
            Get(s"/v0/records${path}start=3&limit=1") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
              page.hasMore shouldBe true
              page.nextPageToken.isDefined shouldBe true
            ...
            Get(s"/v0/records${path}start=4&limit=1") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
              page.hasMore shouldBe false
              page.nextPageToken.isDefined shouldBe false
            ...
            Get(s"/v0/records${path}start=5&limit=1") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
              val page = responseAs[RecordsPage[RecordType]]
              page.hasMore shouldBe false
              page.nextPageToken.isDefined shouldBe false
            ...
            valuesToTest.foreach {
              case TestValues(pageSize, recordCount) =>
                it(s"for pageSize $pageSize and recordCount $recordCount") { param =>
                  ...
                  // Get the page tokens
                  Get(s"/v0/records/pagetokens?limit=$pageSize") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
                    status shouldEqual StatusCodes.OK
                    ...
                    for (pageIndex <- pageTokens.indices) {
                      ...
                      Get(s"/v0/records?pageToken=$token&limit=$pageSize") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
                        status shouldEqual StatusCodes.OK
                        val page = responseAs[RecordsPage[Record]]
            ...
            valuesToTest.foreach {
              case TestValues(pageSize, recordCount) =>
                it(s"for pageSize $pageSize and recordCount $recordCount") { param =>
                  ...
                  for (aspectNumber <- 1 to 2) {
                    Get(s"/v0/records/pagetokens?limit=$pageSize&aspect=$aspectNumber") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
                      status shouldEqual StatusCodes.OK
                      val pageTokens = responseAs[List[String]]
                      ...
                      for (pageIndex <- pageTokens.indices) {
                        ...
                        Get(s"/v0/records?pageToken=$token&limit=$pageSize&aspect=$aspectNumber") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
                          status shouldEqual StatusCodes.OK
                          val page = responseAs[RecordsPage[Record]]
            ...
            valuesToTest.foreach {
              case TestValues(pageSize, recordCount) =>
                it(s"for pageSize $pageSize and recordCount $recordCount") { param =>
                  ...
                  Get(s"/v0/records/pagetokens?limit=$pageSize&${aspectIds.map(id => "aspect=" + id).mkString("&")}") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
                    status shouldEqual StatusCodes.OK
                    val pageTokens = responseAs[List[String]]
                    pageTokens.length shouldEqual recordCount / Math.max(1, pageSize) + 1
                    ...
                    for (pageIndex <- pageTokens.indices) {
                      ...
                      Get(s"/v0/records?pageToken=$token&limit=$pageSize&${aspectIds.map(id => "aspect=" + id).mkString("&")}") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
                        status shouldEqual StatusCodes.OK
                        val page = responseAs[RecordsPage[Record]]
  ...
  def writeTests(role: Role) {
    describe("POST") {
      it("can add a new record") { param =>
        ...
        param.asAdmin(Post("/v0/records", record)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual record.copy(tenantId = Some(SINGLE_TENANT_ID))
        }

        Get("/v0/records/testId") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK

          val recordRes = responseAs[Record]
          recordRes shouldEqual record.copy(tenantId = Some(SINGLE_TENANT_ID))
        }
      }
      ...
      it("supports invalid URL characters in ID") { param =>
        ...
        Get("/v0/records/in%20valid") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("in valid", "testName", Map(), None, Some(SINGLE_TENANT_ID))
        }
      }
      ...
      it("returns 400 if a record with the given ID already exists") { param =>
        ...
        param.asAdmin(Post("/v0/records", updated)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
    ...
    describe("PUT") {
      it("can add a new record") { param =>
        val record = Record("testId", "testName", Map())
        param.asAdmin(Put("/v0/records/testId", record)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual record.copy(tenantId = Some(SINGLE_TENANT_ID))
        }
        ...
        Get("/v0/records") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          ...
          recordsPage.records.head shouldEqual Record("testId", "testName", Map(), None, Some(SINGLE_TENANT_ID))
        }
      }
      ...
      it("can update an existing record") { param =>
        ...
        param.asAdmin(Put("/v0/records/testId", newRecord)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual newRecord.copy(tenantId = Some(SINGLE_TENANT_ID))
        }

        Get("/v0/records/testId") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "newName", Map(), None, Some(SINGLE_TENANT_ID))
        }
      }
      ...
      it("updates the sourcetag of an otherwise identical record without generating events") { param =>
        ...
        Get(s"/v0/records/${record.id}/history") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
      ...
      it("cannot change the ID of an existing record") { param =>
        ...
        param.asAdmin(Put("/v0/records/testId", updated)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.BadRequest
      ...
      it("supports invalid URL characters in ID") { param =>
        val record = Record("in valid", "testName", Map())
        param.asAdmin(Put("/v0/records/in%20valid", record)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual record.copy(tenantId = Some(SINGLE_TENANT_ID))
        }
      ...
      it("can add an aspect") { param =>
        ...
        param.asAdmin(Put("/v0/records/testId", updated)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual updated.copy(tenantId = Some(SINGLE_TENANT_ID))
        }

        Get("/v0/records/testId?aspect=test") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual updated.copy(tenantId = Some(SINGLE_TENANT_ID))
        }
      }
    ...
    describe("PATCH") {
      it("returns an error when the record does not exist") { param =>
        val patch = JsonPatch()
        param.asAdmin(Patch("/v0/records/doesnotexist", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.BadRequest
      ...
      it("can modify a record's name") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "foo", Map(), None, Some(SINGLE_TENANT_ID))
        }
      ...
      it("can add an aspect") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map("test" -> JsObject()), None, Some(SINGLE_TENANT_ID))
        }
      ...
      it("can modify an aspect") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map("test" -> JsObject("foo" -> JsString("baz"))), None, Some(SINGLE_TENANT_ID))
        }
      ...
      it("can add a new property to an aspect") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map("test" -> JsObject("foo" -> JsString("bar"), "newprop" -> JsString("test"))), None, Some(SINGLE_TENANT_ID))
        }
      ...
      it("can remove an aspect") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map(), None, Some(SINGLE_TENANT_ID))
        }

        Get("/v0/records/testId?optionalAspect=test") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map(), None, Some(SINGLE_TENANT_ID))
        }
      }
      ...
      it("can remove a property from an aspect") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map("test" -> JsObject("foo" -> JsString("bar"))), None, Some(SINGLE_TENANT_ID))
        }
      ...
      it("supports Move within an aspect") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map("test" -> JsObject("bar" -> JsString("bar"))), None, Some(SINGLE_TENANT_ID))
        }
      ...
      it("supports Copy within an aspect") { param =>
        ...
        param.asAdmin(Patch("/v0/records/testId", patch)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map("test" -> JsObject("foo" -> JsString("bar"), "bar" -> JsString("bar"))), None, Some(SINGLE_TENANT_ID))
        }
      ...
      it("evaluates Test operations") { param =>
        ...
        param.asAdmin(Post("/v0/aspects", A)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
        }
        ...
        param.asAdmin(Patch("/v0/records/testId", patchSuccess)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
          responseAs[Record] shouldEqual Record("testId", "testName", Map("A" -> JsObject("foo" -> JsString("bar"))), None, Some(SINGLE_TENANT_ID))
        }

        val patchFail = JsonPatch(Test(Pointer.root / "aspects" / "A" / "foo", JsString("not this value")))
        param.asAdmin(Patch("/v0/records/testId", patchFail)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.BadRequest
      ...
      it("does not support Move between aspects") { param =>
        ...
        param.asAdmin(Post("/v0/aspects", B)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
          status shouldEqual StatusCodes.OK
        }
    ...
    describe("DELETE") {
      describe("by id") {
        it("can delete a record without any aspects") { param =>
          ...
          param.asAdmin(Delete("/v0/records/without")) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
        ...
        it("returns 200 and deleted=false when asked to delete a record that doesn't exist") { param =>
          param.asAdmin(Delete("/v0/records/doesnotexist")) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
        ...
        it("can delete a record with an aspect") { param =>
          ...
          param.asAdmin(Delete("/v0/records/with")) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
            responseAs[DeleteResult].deleted shouldBe true
          }
        }
      ...
      describe("based on a source tag") {
        ...
        it("deletes only records with the correct source and without the specified tag") { param =>
          ...
          all.foreach(record => param.asAdmin(Post("/v0/records", record)) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
          })
          ...
          param.asAdmin(Delete("/v0/records?sourceTagToPreserve=righttag&sourceId=right")) ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
            status shouldEqual StatusCodes.OK
          ...
          List("wrongtag-rightsource", "notag-rightsource").foreach { recordId =>
            Get(s"/v0/records/$recordId") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.NotFound
            }

            Get(s"/v0/records/$recordId/history") ~> addSingleTenantIdHeader ~> param.api(role).routes ~> check {
              status shouldEqual StatusCodes.OK
        ...
        it("returns Accepted HTTP code if delete is taking too long") { param =>
          ...
          param.asAdmin(Delete("?sourceTagToPreserve=righttag&sourceId=right")) ~> addSingleTenantIdHeader ~> mockedApi.route ~> check {
