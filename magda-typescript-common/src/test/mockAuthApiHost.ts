...
export default class mockAuthApiHost{
    ...
    {
        if(!authApiUrl) throw new Error("authApiUrl is required to run mockAuthApiHost");
        if(!jwtSecret) throw new Error("jwtSecret is required to run mockAuthApiHost");
        if(!defaultAdminUserId) throw new Error("defaultAdminUserId is required to run mockAuthApiHost");
    ...
    {
        const userId = getUserId(req, this.jwtSecret).valueOrThrow(new AuthError("Unauthorised",401));
        if(!requiredAdminAccess) return;
        if(userId!=this.defaultAdminUserId) throw new AuthError("Can only be accessed by Admin users", 403);
    }
        ...
        this.scope
            ...
            .reply(function(this:any, uri,requestBody){
                ...
                try{
                    ...
                    if(!source || !sourceId) return [500,"Missing query parameter"];
                    const records = mockUserDataStore.getRecordBySourceAndSourceId(source, sourceId);
        ...
        this.scope
            ...
            .reply(function(this:any, uri,requestBody){
                ...
                try{
                    ...
                    const records = mockUserDataStore.getRecordByUserId(userId).map(record=>({
                        id: record.id,
