...
class ElasticSearchIndexer(
  ...
  private lazy val indexQueue: SourceQueue[(DataSet, Promise[Unit])] =
    Source.queue[(DataSet, Promise[Unit])](indexingQueueBufferSize, OverflowStrategy.backpressure)
      ...
      .mapAsync(1) { batch =>
        ...
        val bulkDef = bulk(batch.flatMap { case (indexRequests, _) => indexRequests })
  ...
  private def updateIndices(client: ElasticClient, definitionPairs: Seq[(Option[Seq[IndexMappings]], IndexDefinition)]): Future[Object] =
  ...
  private def createSnapshotRepo(client: ElasticClient, definition: IndexDefinition) = {
  ...
  private def createSnapshot(client: ElasticClient, definition: IndexDefinition): Future[Unit] = {
   ...
   */
  private def buildDatasetIndexDefinition(rawDataSet: DataSet): Seq[IndexRequest] = {
