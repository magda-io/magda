...
export default class ProjectOpenData implements ConnectorSource {
    ...
    public getJsonDataset(id: string): Promise<any> {
        return this.dataPromise.then((response: any) => {
            ...
            return response.dataset.filter((dataset: any) => dataset.identifier === id)[0];
        });
    }
    ...
    public searchDatasetsByTitle(title: string, maxResults: number): AsyncPage<any[]> {
        const promise = this.dataPromise.then((response: any) => {
            ...
            return response.dataset.filter((dataset: any) => dataset.title.toLowerCase().indexOf(lowercaseTitle) >= 0).slice(0, maxResults);
        });
